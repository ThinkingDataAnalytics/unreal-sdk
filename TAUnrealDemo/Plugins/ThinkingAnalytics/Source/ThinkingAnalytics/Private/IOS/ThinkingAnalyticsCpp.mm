...
void ThinkingAnalyticsCpp::ta_initializeEncryptInstance(FString appid, FString serverurl, TAMode mode, bool bEnableLog, FString timeZone, FString version, FString EncryptPublicKey, int EncryptVersion, FString SymmetricEncryption, FString AsymmetricEncryption)
{
    if ( bEnableLog ) {
        [ThinkingAnalyticsSDK setLogLevel:TDLoggingLevelDebug];
    }
    ...
    const char *encryptPublicKeyValue = TCHAR_TO_UTF8(*EncryptPublicKey);
    NSString *encryptPublicKeyStr = encryptPublicKeyValue != NULL ? [NSString stringWithUTF8String:encryptPublicKeyValue] : nil;
    const char *asymmetricEncryptionValue = TCHAR_TO_UTF8(*AsymmetricEncryption);
    NSString *asymmetricEncryptionStr = asymmetricEncryptionValue != NULL ? [NSString stringWithUTF8String:asymmetricEncryptionValue] : nil;
    const char *symmetricEncryptionVaule = TCHAR_TO_UTF8(*SymmetricEncryption);
    NSString *symmetricEncryptionStr = symmetricEncryptionVaule != NULL ? [NSString stringWithUTF8String:symmetricEncryptionVaule] : nil;
...
void ThinkingAnalyticsCpp::ta_setAutoTrackEventListener(TAAutoTrackEventRetValDelegate Del, TArray<FString> EventTypeList, FString AppId)
{
...
void ThinkingAnalyticsCpp::ta_user_unique_append(FString properties, FString appid) 
{
...
void ThinkingAnalyticsCpp::ta_set_trackStatus(FString status, FString appid)
{
    ThinkingAnalyticsSDK *instance = getCurrentInstance(appid);
    if ( instance )
    {
...
void ThinkingAnalyticsCpp::ta_enableThirdPartySharing(int types, FString appId)
{
    ThinkingAnalyticsSDK *instance = getCurrentInstance(appId);
    if( instance )
    {
...
void ThinkingAnalyticsCpp::ta_enableThirdPartySharingWithCustomProperties(int typeList, FString properties, FString appId)
{
    ThinkingAnalyticsSDK *instance = getCurrentInstance(appId);
    if( instance )
    {
